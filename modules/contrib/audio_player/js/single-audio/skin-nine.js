(function ($, Drupal, drupalSettings) {
  ...
  Drupal.behaviors.audio_player_skin_nine = {
    attach: function (context, settings) {
      // Use once to ensure the script runs only once per element.
      once('audio_player_skin_nine', '.audio-player.skin-nine', context).forEach(function (playerElement) {

        const $player = $(playerElement);

        const $audio = $player.find('.audio-player__my-audio');
        const audio = $audio[0]; // Get the native DOM element for media events and properties

        const $playPauseBtn = $player.find('.audio-player__play-pause-btn');
        const $playIcon = $playPauseBtn.find('.audio-player__play-icon');
        const $pauseIcon = $playPauseBtn.find('.audio-player__pause-icon');
        const $rewindBtn = $player.find('.audio-player__rewind-btn');
        const $fastForwardBtn = $player.find('.audio-player__fast-forward-btn');
        const $muteUnmuteBtn = $player.find('.audio-player__mute-unmute-btn');
        const $volumeUpIcon = $muteUnmuteBtn.find('.audio-player__volume-up-icon');
        const $volumeMuteIcon = $muteUnmuteBtn.find('.audio-player__volume-mute-icon');
        const $volumeSlider = $player.find('.audio-player__volume-slider');
        const $progressBar = $player.find('.audio-player__progress-bar');
        const $bufferedBar = $player.find('.audio-player__buffered-bar'); // NEW: Get buffered bar element
        const $progressContainer = $player.find('.audio-player__progress-container');
        const $currentTimeSpan = $player.find('.audio-player__current-time');
        const $totalTimeSpan = $player.find('.audio-player__total-time');
        const $songNameText = $player.find('.audio-player__song-name');
        const $artistNameText = $player.find('.audio-player__artist-name');
        const $playbackSpeedSelect = $player.find('.audio-player__playback-speed-select');

        let isPlaying = false;
        let initialVolume = audio.volume;
        let isSeeking = false; // NEW: Flag for seeking to prevent timeupdate interference
        ...
        $audio.on('timeupdate', () => {
          ...
          updateBufferedBar(); // NEW: Call buffer update on timeupdate for consistency
        });
        ...
        // NEW: Update buffered bar as audio buffers (crucial event for buffering)
